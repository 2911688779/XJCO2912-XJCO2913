...
@Api(tags = "防汛-区信息")
@RestController
@RequestMapping(value = "/api/fangxun/qu")
public class QuController {
    private static final Logger logger = LoggerFactory.getLogger(QuController.class);

    @Autowired
    private QuService actionService;

    @ApiOperation(value = "save")
    @PostMapping(value = "/save")
    public ResponseUtil save(@RequestBody Qu action){

        Qu result = actionService.saveAction(action);
        return ResponseUtil.builder().success(true).data(result).build();
    }

    @ApiOperation(value = "update")
    @PostMapping(value = "/update")
    public ResponseUtil updateAction(@RequestBody Qu action){
    ...
    public ResponseUtil findAll(){
        List<Qu> actionsList = actionService.findAll();
    ...
    public @ResponseBody ResponseUtil findById(@PathVariable String id){
        Qu action = actionService.findById(id);
