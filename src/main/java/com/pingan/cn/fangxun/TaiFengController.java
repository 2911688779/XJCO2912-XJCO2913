...
@Api(tags = "防汛-台风")
@RestController
@RequestMapping(value = "/api/fangxun/taifeng")
public class TaiFengController {
    private static final Logger logger = LoggerFactory.getLogger(TaiFengController.class);

    @Autowired
    private TaiFengService actionService;

    @ApiOperation(value = "save")
    @PostMapping(value = "/save")
    public ResponseUtil save(@RequestBody TaiFeng action){

        TaiFeng result = actionService.saveAction(action);
        return ResponseUtil.builder().success(true).data(result).build();
    }

    @ApiOperation(value = "update")
    @PostMapping(value = "/update")
    public ResponseUtil updateAction(@RequestBody TaiFeng action){
    ...
    public ResponseUtil findAll(){
        List<TaiFeng> actionsList = actionService.findAll();
    ...
    public @ResponseBody ResponseUtil findById(@PathVariable String id){
        TaiFeng action = actionService.findById(id);
