...
@Api(tags = "防汛-抢险")
@RestController
@RequestMapping(value = "/api/fangxun/qiangxian")
public class QiangXianController {
    private static final Logger logger = LoggerFactory.getLogger(QiangXianController.class);

    @Autowired
    private QiangXianService actionService;

    @ApiOperation(value = "save")
    @PostMapping(value = "/save")
    public ResponseUtil save(@RequestBody QiangXian action){

        QiangXian result = actionService.saveAction(action);
        return ResponseUtil.builder().success(true).data(result).build();
    }

    @ApiOperation(value = "update")
    @PostMapping(value = "/update")
    public ResponseUtil updateAction(@RequestBody QiangXian action){
    ...
    public ResponseUtil findAll(){
        List<QiangXian> actionsList = actionService.findAll();
    ...
    public @ResponseBody ResponseUtil findById(@PathVariable String id){
        QiangXian action = actionService.findById(id);
