...
@Api(tags = "罐区-水库")
@RestController
@RequestMapping(value = "/api/shuiku")
public class ShuikuController {
    private static final Logger logger = LoggerFactory.getLogger(ShuikuController.class);

    @Autowired
    private ShuikuService actionService;

    @ApiOperation(value = "save")
    @PostMapping(value = "/save")
    public ResponseUtil save(@RequestBody Shuiku action){

        Shuiku result = actionService.saveAction(action);
        return ResponseUtil.builder().success(true).data(result).build();
    }

    @ApiOperation(value = "update")
    @PostMapping(value = "/update")
    public ResponseUtil updateAction(@RequestBody Shuiku action){
    ...
    public ResponseUtil findAll(){
        List<Shuiku> actionsList = actionService.findAll();
    ...
    public @ResponseBody ResponseUtil findById(@PathVariable String id){
        Shuiku action = actionService.findById(id);
