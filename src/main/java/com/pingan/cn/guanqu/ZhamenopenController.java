...
@Api(tags = "罐区-闸门open")
@RestController
@RequestMapping(value = "/api/zhamenopen")
public class ZhamenopenController {
    private static final Logger logger = LoggerFactory.getLogger(ZhamenopenController.class);

    @Autowired
    private ZhamenopenService actionService;

    @ApiOperation(value = "save")
    @PostMapping(value = "/save")
    public ResponseUtil save(@RequestBody Zhamenopen action){

        Zhamenopen result = actionService.saveAction(action);
        return ResponseUtil.builder().success(true).data(result).build();
    }

    @ApiOperation(value = "update")
    @PostMapping(value = "/update")
    public ResponseUtil updateAction(@RequestBody Zhamenopen action){
    ...
    public ResponseUtil findAll(){
        List<Zhamenopen> actionsList = actionService.findAll();
    ...
    public @ResponseBody ResponseUtil findById(@PathVariable String id){
        Zhamenopen action = actionService.findById(id);
