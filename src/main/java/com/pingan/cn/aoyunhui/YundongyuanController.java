...
@Api(tags = "奥运会-运动员")
@RestController
@RequestMapping(value = "/api/yundongyuan")
public class YundongyuanController {
    private static final Logger logger = LoggerFactory.getLogger(YundongyuanController.class);

    @Autowired
    private YundongyuanService actionService;

    @ApiOperation(value = "save")
    @PostMapping(value = "/save")
    public ResponseUtil save(@RequestBody Yundongyuan action){

        Yundongyuan result = actionService.saveAction(action);
        return ResponseUtil.builder().success(true).data(result).build();
    }

    @ApiOperation(value = "update")
    @PostMapping(value = "/update")
    public ResponseUtil updateAction(@RequestBody Yundongyuan action){
    ...
    public ResponseUtil findAll(){
        List<Yundongyuan> actionsList = actionService.findAll();
    ...
    public @ResponseBody ResponseUtil findById(@PathVariable String id){
        Yundongyuan action = actionService.findById(id);
