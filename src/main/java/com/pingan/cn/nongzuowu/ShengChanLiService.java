...
@Service("shengChanLiService")
public class ShengChanLiService {
    @Autowired
    private ShengChanLiDao actionDao;
    private static String dateFormat = "yyyy-MM-dd HH:mm:ss";
    private static SimpleDateFormat sdf = new SimpleDateFormat(dateFormat);

    public ShengChanLi saveAction(ShengChanLi action){
        PinZhong save = null;
        try {
    ...
    public boolean update(ShengChanLi action){
        if(action == null || action.getId() == null){
            return  false;
        }
        Optional<ShengChanLi> byId = actionDao.findById(action.getId());
        if (byId.isPresent()){
            ShengChanLi oldAction = actionDao.findById(action.getId()).get();
    ...
    public List<ShengChanLi> findAll(){
        return actionDao.findAll();
    }

    public ShengChanLi findById(Integer id){
    ...
    public boolean deleteBatch(Integer[] ids){
        ArrayList<ShengChanLi> arrayList = new ArrayList<>();
        for (Integer id:ids) {
            ShengChanLi entity = findById(id);
            if (entity != null){
