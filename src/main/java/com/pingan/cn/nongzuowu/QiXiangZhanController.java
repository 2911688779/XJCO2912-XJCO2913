...
@Api(tags = "农作物-气象站")
@RestController
@RequestMapping(value = "/api/qiXiangZhan")
public class QiXiangZhanController {

    @Autowired
    private QiXiangZhanService actionService;

    @ApiOperation(value = "save")
    @PostMapping(value = "/save")
    public ResponseUtil save(@RequestBody QiXiangZhan action){

        QiXiangZhan result = actionService.saveAction(action);
        return ResponseUtil.builder().success(true).data(result).build();
    }

    @ApiOperation(value = "update")
    @PostMapping(value = "/update")
    public ResponseUtil updateAction(@RequestBody QiXiangZhan action){
    ...
    public ResponseUtil findAll(){
        List<QiXiangZhan> actionsList = actionService.findAll();
    ...
    public @ResponseBody ResponseUtil findById(@PathVariable Integer id){
        QiXiangZhan action = actionService.findById(id);
